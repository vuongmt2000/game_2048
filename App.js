...
const App = () => {
  ...
  const getData = async () => {
    try {
      const coreValue = await AsyncStorage.getItem('score')
      const heightScoreValue = await AsyncStorage.getItem('heightScore')
      const countValue = await AsyncStorage.getItem('count');
      if (heightScoreValue) {
        setHeightScore(parseInt(heightScoreValue))
      }
      if (coreValue) {
        setScore(parseInt(coreValue))
      }
      if (countValue) {
  ...
  const save = useCallback(
    async () => {
      try {
        ...
        await AsyncStorage.setItem('heightScore', heightScore.toString())
        await AsyncStorage.setItem('count', JSON.stringify(count));
      } catch (e) {
        // saving error
      }
    },
    [score, heightScore, count],
  )
